...
State_unigram = Death_records %>% group_by(State) %>% summarise(MannerOfDeath=paste(MannerOfDeath,collapse=" "))
State_unigram = State_unigram %>% unnest_tokens(tokens, MannerOfDeath, to_lower = TRUE)
...
plot(state_graph_weighted,layout=layout.fruchterman.reingold,  edge.arrow.size=.7,
     ...
     edge.color="lightblue3", vertex.color="indianred2", vertex.frame.color = "black",
     frame = 'TRUE',main = "State Network by Manner of death",
